  ...
  return next(action)
    ...
    .catch(error => {
      if (action.meta && action.meta.errorMessage) {
        ...
        if (!(response.status === 401 && (error.message === '' || (data && data.path && data.path.includes('/admin-api/account'))))) {
          let i;
            ...
            case 400: {
              ...
              if (errorHeader) {
                ...
                for (i = 0; i < fieldErrors.length; i++) {
                  ...
                  const fieldName = translate(`greatEscapeAdminUiApp.${fieldError.objectName}.${convertedField}`);
      ...
      } else if (error && error.config && error.config.url === 'admin-api/account' && error.config.method === 'get') {
        /* eslint-disable no-console */
        console.log('Authentication Error: Trying to access url admin-api/account with GET.');
